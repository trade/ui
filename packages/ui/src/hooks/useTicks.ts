 ...
 * ```
 *
 * Environment notes: the queue is client-only — a server render uses the initial value, which is
 * what SSR wants. With no `requestAnimationFrame` available the timeout path is used, and with
 * neither, the flush runs synchronously rather than never.
 */
export function useTicks<T>(initial: T, options: UseTicksOptions = {}): [T, (update: Updater<T>) => void] {
  ...
  const push = useCallback(
    (update: Updater<T>) => {
      ...
      if (handle.current !== null) return; // a flush is already scheduled for this window
  ...
  // Unmount cancels whatever is pending, so a flush can never land on an unmounted component.
  useEffect(
    () => () => {
