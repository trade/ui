...
- **Phase A — static (19):** package contract (zero runtime deps, React peer, `sideEffects:
...
- **Phase B — jsdom interaction (14):** tab click + ArrowRight roving tabindex; dialog
...
### `scripts/verify-demo.mjs` — 10 checks
...
### `scripts/browser-suite.mjs` — 12 checks × 6 combos = 72
...
failures at p95 = 22). Throttle-looking readings (p95 > 100 ms) retry up to 3× and keep the
better attempt — runner noise must not fail real regressions, but it also must not mask them.
...
4.34 / 4.06 kB, comfortably under.

## CI and the local loop

`npm run ci` = `check:contract → build → check:contrast → check:style → demo → harness:build →
size → verify → verify:demo → verify:browser`, all non-zero-exit. CI (.github/workflows/ci.yml,
Node 22) runs four jobs: **build** (the static checks + dist artifact), **size**, **verification**
(verify + demo + all three browser engines; uploads `verification/` evidence even on failure),
...
- Demo-level behaviour → `scripts/verify-demo.mjs`.
- New token pair → `check-contrast.mjs` `PAIRS`. New CSS convention → `check-style.mjs`.
- New package promise → `check-contract.mjs`.
